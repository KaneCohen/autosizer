      ...
      if (typeof events === 'string') {
        ...
        for (i = 0; i < events.length; i++) {
          ...
          if (handler) {
            ...
            for (k = 0; k < prevEvents[ev].length; k++) {
              el.removeEventListener(ev.split('.')[0], prevEvents[ev][k]);
              prevEvents[k] = prevEvents[ev].slice(k + 1);
            }
          } else if (ev[0] === '.') {
            for (k in prevEvents) {
              if (k.indexOf(ev) > -1) {
                for (j = 0; j < prevEvents[k].length; j++) {
                  el.removeEventListener(k.split('.')[0], prevEvents[k][j]);
